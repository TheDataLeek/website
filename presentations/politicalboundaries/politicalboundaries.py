...
def simulated_annealing(system, numdistricts, precision, animate, makegif):
    ...
    k = 0.15  # Larger k => more chance of randomly accepting
    ...
    with Halo(text='Running Algorithm', spinner='dots') as spinner:
        while True:
            try:
                ...
                if dv > 0 or random.random() < math.exp(dv / (k * Tvals[iter_num])):
                    solution = new_solution
                    cval = solution.value
                    history.append(new_solution)
                    spinner.text = f'Iteration {iter_num}, Value {cval:0.03f}'
                    if dv > 0:
                        iterations_since_increase = 0
                else:
                ...
                if ((iterations_since_increase > STICKY_NUM) and
                    ...
                    break
                iter_num += 1
                if iter_num >= len(Tvals):
                    break
            except (KeyboardInterrupt, EOFError) as e:
                print('User Exit, Aborting.')
                break
        spinner.succeed()
...
def get_good_start(system, numdistricts):
    ...
    for i in tqdm(range(1000)):
        ...
        if new_solution.value > cval:
            solution = new_solution
            cval = new_solution.value
            if cval > 25:
                break
    print(f'Starting with Solution[{cval}]')
    return solution
    ...
    with Halo(text='Running Algorithm', spinner='dots') as spinner:
        while True:
            try:
                ...
                if len(top_history) == 0 or solutions[0].value > cval:
                    top_history.append(solutions[0])
                    spinner.text = f'Current Generation Top Solution: {solutions[0].value:0.03f}'
                    iterations_since_increase = 0
                    cval = solutions[0].value
                else:
                    iterations_since_increase += 1
                if ((iterations_since_increase > STICKY_NUM) and
                        (cval >= TARGET_VALUE)):
